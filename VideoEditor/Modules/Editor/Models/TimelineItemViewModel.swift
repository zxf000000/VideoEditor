...
class TimelineItemViewModel {
    var widthInTimeline: CGFloat? {
        get {
            return GetWidthFor(timeRange: (timelineItem?.timeRange)!, scaleFactor: TIMELINE_SINGLESECOND_WIDTH)
        }
    }
    ...
    init(timeline: TimeLineItem) {
        ...
        maxWidthInTimeline = GetWidthFor(timeRange: maxTimeRange, scaleFactor: TIMELINE_SINGLESECOND_WIDTH)
    }
    func udpateTimelineItem() {
        if positionInTimeline?.x ?? 0 > 0 {
            let startTime = GetTime(for: positionInTimeline?.x ?? 0, scaleFactor: TIMELINE_SINGLESECOND_WIDTH)
            timelineItem?.startTimeInTimeLine = startTime
        }
        timelineItem?.timeRange = GetTimeRange(for: widthInTimeline!, scaleFactor: TIMELINE_SINGLESECOND_WIDTH)
    }
}
